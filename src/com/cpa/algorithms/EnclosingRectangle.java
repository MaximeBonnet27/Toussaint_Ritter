...
public class EnclosingRectangle {
	...
	public static Rectangle computeToussaint(ArrayList<Point.Double> hull) {
		...
		while (!hullScanned) {
			/*
			 * Pour chaque point, on calcule le cosinus entre sa ligne de
			 * support et le prochain coté dans l'enveloppe convexe. De plus, on
			 * garde en mémoire la valeur du cosinus maximum, et l'index du
			 * point corresepondant.
			 */
			cos_theta_i = GeometryTools.cos(
					support_i,
					new Line(hull.get(index_i), hull.get((index_i + 1)
							% hull.size())));
			cos_theta_j = GeometryTools.cos(
					support_j,
					new Line(hull.get(index_j), hull.get((index_j + 1)
							% hull.size())));
			...
			cos_theta_k = GeometryTools.cos(
					support_k,
					new Line(hull.get(index_k), hull.get((index_k + 1)
							% hull.size())));
			if (cos_theta_k > cos_theta_max) {
				cos_theta_max = cos_theta_k;
				index_cos_max = index_k;
			}
			cos_theta_l = GeometryTools.cos(
					support_l,
					new Line(hull.get(index_l), hull.get((index_l + 1)
							% hull.size())));
			...
			/*
			 * Quatrième étape : On fait avancer le point correspondant à
			 * l'angle minimum. Sa ligne support est alors confondue avec le
			 * prochain coté de l'enveloppe convexe, et on détermine les trois
			 * autres lignes de support en fonction de celle-ci (orthogonale /
			 * inverse)
			 */
